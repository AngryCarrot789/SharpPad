...
public class RateLimitedDispatchAction : IDispatchAction
{
    ...
    public static RateLimitedDispatchAction ForDispatcherAsync(Func<Task> callback, TimeSpan minInterval) => ForDispatcherAsync(callback, minInterval, DispatcherPriority.Send);

    public static RateLimitedDispatchAction ForDispatcherAsync(Func<Task> callback, TimeSpan minInterval, DispatcherPriority priority) => ForDispatcherAsync(callback, minInterval, Dispatcher.UIThread, priority);

    public static RateLimitedDispatchAction ForDispatcherAsync(Func<Task> callback, TimeSpan minInterval, Dispatcher dispatcher) => ForDispatcherAsync(callback, minInterval, dispatcher, DispatcherPriority.Send);

    public static RateLimitedDispatchAction ForDispatcherAsync(Func<Task> callback, TimeSpan minInterval, Dispatcher dispatcher, DispatcherPriority priority)
    {
        Validate.NotNull(callback, nameof(callback));
    ...
    public static RateLimitedDispatchAction ForDispatcherSync(Action callback, TimeSpan minInterval) => ForDispatcherSync(callback, minInterval, DispatcherPriority.Send);

    public static RateLimitedDispatchAction ForDispatcherSync(Action callback, TimeSpan minInterval, DispatcherPriority priority) => ForDispatcherSync(callback, minInterval, Dispatcher.UIThread, priority);

    public static RateLimitedDispatchAction ForDispatcherSync(Action callback, TimeSpan minInterval, Dispatcher dispatcher) => ForDispatcherSync(callback, minInterval, dispatcher, DispatcherPriority.Send);

    public static RateLimitedDispatchAction ForDispatcherSync(Action callback, TimeSpan minInterval, Dispatcher dispatcher, DispatcherPriority priority)
    {
        Validate.NotNull(callback, nameof(callback));
        Validate.NotNull(dispatcher, nameof(dispatcher));
        ...
        return new RateLimitedDispatchAction(() => dispatcher.InvokeAsync(callback, priority).GetTask(), minInterval);
    }
    ...
    private async Task TaskMain()
    {
        ...
        do
        {
            ...
            {
                Dispatcher.UIThread.Post(() => throw new Exception("Exception while awaiting operation", e));
            }
            finally
            {
