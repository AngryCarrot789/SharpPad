...
namespace SharpPad.Avalonia.Tasks;
...
public class TaskManager : IDisposable
{
    ...
    internal static Task InternalPreActivateTask(TaskManager taskManager, ActivityTask task)
    {
        ...
        return Dispatcher.UIThread.InvokeAsync(() => InternalOnTaskStarted(taskManager, task)).GetTask();
    }
    ...
    internal static Task InternalOnActivityCompleted(TaskManager taskManager, ActivityTask task, int state)
    {
        ...
        return Dispatcher.UIThread.InvokeAsync(() => InternalOnTaskCompleted(taskManager, task, state)).GetTask();
    }
