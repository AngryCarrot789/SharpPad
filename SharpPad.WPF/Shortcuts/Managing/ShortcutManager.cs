...
namespace SharpPad.WPF.Shortcuts.Managing
{
    ...
    public abstract class ShortcutManager
    {
        ...
        public IEnumerable<GroupedShortcut> GetShortcutsByCommandId(string cmdId, CmdToShortcutFlags flags = CmdToShortcutFlags.Both)
        {
            ...
            switch (flags)
            {
                case CmdToShortcutFlags.Keyboard: return value.Where(x => x.Shortcut is KeyboardShortcut);
                case CmdToShortcutFlags.Mouse: return value.Where(x => x.Shortcut is MouseShortcut);
                case CmdToShortcutFlags.Both: return value;
