...
using SharpPad.WPF.Notepads.Commands;
using SharpPad.WPF.Properties;
using SharpPad.WPF.Services.Files;
using SharpPad.WPF.Services.Messages;
using SharpPad.WPF.Services.WPF.Files;
using SharpPad.WPF.Services.WPF.Messages;
        ...
        public void OnApplicationLoaded(Notepad notepad, string[] args)
        {
            ...
            else
            {
                ...
                TaskManager.Instance.RunTask(async () =>
                {
                    ...
                    const int duration_s = 1, updates = (int) (duration_s / interval_s);
                    const double progPerTick = 1.0 / updates;
                    TimeSpan delayTime = TimeSpan.FromSeconds(interval_s);

                    for (int i = 0; i < updates; i++)
                    {
                        await System.Threading.Tasks.Task.Delay(delayTime);
                        prog.OnProgress(progPerTick);
                    }
                });
#else
