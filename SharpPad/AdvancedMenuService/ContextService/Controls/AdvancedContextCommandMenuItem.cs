    ...
    public class AdvancedContextCommandMenuItem : AdvancedContextMenuItem {
        ...
        public void UpdateCanExecuteVisual(IContextData ctx) {
            ...
            if (this.IsExecuting) {
                ...
                Executability state = !string.IsNullOrWhiteSpace(cmdId) ? CommandManager.Instance.CanExecute(cmdId, ctx, true) : Executability.Invalid;
            ...
            try {
                ...
                if (!Debugger.IsAttached) {
                    IoC.MessageService.ShowMessage(
                        ...
                        "SharpPad may or may not crash now, but you should probably restart and save just in case",
                        e.GetToString());
                }
            }
            finally {
